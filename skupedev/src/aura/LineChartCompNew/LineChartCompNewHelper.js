({
    ...
    getTableData1: function(component, filters, lCycle, brand, startDt, endDt, strType) {
        ...
        actionData.setCallback(this, function(df) {
            ...
            if (stateData == "SUCCESS") {
                ...
                if (!$A.util.isEmpty(dataResults) && !$A.util.isUndefined(dataResults)) {
                    //alert(JSON.stringify(dataResults['Actuals'].datWrap));
                    var container = component.find("lstCatWrapId");
                	$A.createComponent("c:ListDataTableComp",
						{
							"lstTHeadWrap": dataResults['Actuals'].headWrap,
							"lstCatWrapper": dataResults['Actuals'].datWrap,
							"actuals" : true,
							"pProjections" : false,
							"fProjections" :false,
							"tHeading" : "Actual",
							"passMethod" : component.getReference("c.showOppmodal")
						},
					function(comp){
					...
					var container1 = component.find("lstMainProjWrapId");
                	$A.createComponent("c:ListDataTableComp",
						{
							"lstTHeadWrap": dataResults['Actuals'].headWrap,
							"lstCatWrapper": dataResults['Past Projections'].datWrap,
							"actuals" : false,
							"pProjections" : true,
							"fProjections" :false,
							"tHeading" : "Projection",
					...
					function(comp){
						container1.set("v.body",[comp]); 
						//$A.get("e.c:renderdynamicvals").fire();
						//helper.hideSpinner(cmp);
					});
					
					var container2 = component.find("lstProjWrapId");
                	$A.createComponent("c:ListDataTableComp",
						{
							"lstTHeadWrap": dataResults['Future Projections'].headWrap,
							"lstCatWrapper": dataResults['Future Projections'].datWrap,
							"actuals" : false,
							"pProjections" : false,
							"fProjections" : true,
							"tHeading" : "Projection",
	...
	getSKUDynamicData : function(component,brandval,status,group) {
        ...
        action.setParams({
                            "cat":	component.get("v.categoryStr"), //document.getElementById("CATEGORY").innerHTML,
                            "strgroup": group,	//document.getElementById("GROUP").innerHTML,
							"mon": component.get("v.monthStr"),	//document.getElementById("MONTH").innerHTML,
							"status":status,	//document.getElementById("STATUS").innerHTML,
							"selBrand": brandval,
							"strType": component.get("v.lifecycleStr"),	//document.getElementById("lifecycle").innerHTML
                        });
            ...
            action.setCallback(this,function(a){
                ...
                if(state == "SUCCESS"){
                    ...
                    if(!$A.util.isEmpty(result) && !$A.util.isUndefined(result))
                    {
                    	...
                    	/*component.set("v.renderPopup1", true);*/
                    	var container = component.find("popUpContainer");
                    	$A.createComponent("c:ListPopUpComp",
							{
								"lstSKUs":result,
								"CATEGORY":component.get("v.categoryStr"),
								"MONTH":component.get("v.monthStr"),
								"lifecycle":component.get("v.lifecycleStr")
							},
						function(comp){
							container.set("v.body",[comp]); 
							//$A.get("e.c:renderdynamicvals").fire();
