({
    ...
    initializeTable : function(cmp,event,helper,rows){
        ...
        if(initConfig.itemsPerPage) {
            if(pageLimitIdx > -1) {
                cmp.set("v.limit",initConfig.itemsPerPage);
            }
        } 
        ...
        if(popoverActions.length > 0){
            ...
            for (var i = 0; i < popoverActions.length; i++) {
                ...
                if(!menuAction.label) {
                    ...
                    //btnDiv.innerHTML += span.outerHTML;
                    ...
                    li.addEventListener('click',$A.getCallback(function(evt){
                        ...
                        if(ele.length > 0){
                            for(var i = 0;i < ele.length;i++) {
                                //ele[i].className = 'slds-dropdown-trigger slds-dropdown-trigger--click'; 
    ...
    identifyPosition:function(cmp,event){
        var tbodyNode = event.currentTarget;
        var tableNode = cmp.find("dtTable").getElement();
        
        var parentHeight = tableNode.clientHeight;
        var relativeYPosition = tbodyNode.getBoundingClientRect().top - tableNode.getBoundingClientRect().top;
        
        if(parentHeight < relativeYPosition + 100 ){
            tbodyNode.childNodes[1].className = 'slds-dropdown slds-dropdown--bottom slds-dropdown--right';
        }
    },
